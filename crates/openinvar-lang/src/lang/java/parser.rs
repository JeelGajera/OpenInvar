...
pub fn parse_file(root: &Path, path: &Path) -> Result<ParsedFile, String> {
    ...
    // A parse error is recorded rather than fatal: one unparseable file should
    // not cost the analysis of every other file in the repository, and the
    // diagnostic is what tells a reader the region is incomplete.
